...
- `migrations/20260826020000_rls_policies.sql` — enables Row-Level Security on every table and adds the policies scoping each to the caller's own tenant. This is a backstop, not the primary access path: the Node/Express API is expected to connect with a privileged role that bypasses RLS. These policies matter if a dealer/admin session ever queries Postgres directly through Supabase's authenticated client — and they fully lock out the `anon` role on every table, since the public, unauthenticated actions (a friend submitting a lead, gift card issuance) are meant to go through the service-role API, not a direct client connection. See the comments at the top of the migration for the reasoning per table.

No functions or triggers beyond the two RLS helper functions (`current_tenant_id()`, `is_admin()`) yet.
