...
- `migrations/20260906010000_submit_referral_dealer_notification.sql` — extends `submit_referral()` (drop + recreate, since `CREATE OR REPLACE` can't change return columns) to also return the inviting dealer's email/name, the referring customer's name, and the tenant's name/sender config, alongside the existing `id`/`submitted_at`. `anon` still has no table access at all — this is the only way the API's public lead-submission route (see [`../api/README.md`](../api/README.md)) can know *which* dealer to notify without a second query it has no privilege to run. A customer with no inviting dealer on record comes back with a null `dealer_email` rather than an error.
...
- The extended `submit_referral()`: called directly as `anon` in SQL for a customer with an inviting dealer (correctly returns that dealer's real email/name and the referrer's name) and one with none (`dealer_email` comes back null, not an error). Full live-email verification, including the actual dealer notification arriving with the right content, is in [`../api/README.md`](../api/README.md)'s "How the dealer gets notified of a new lead" and "Verified" sections.
