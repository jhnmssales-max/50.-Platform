...
// API key and funding source (their own card), so a reward is always
// funded by the company whose customer earned it, never a shared platform
// account. The key is encrypted at rest (pgcrypto, see the migration) and
// is write-only from here on out: GET never returns it, only whether one
// is on file. Admin-only, since this is a company-wide payment setting,
// same as everything else on the tenants row.
// ---------------------------------------------------------------------------
...
router.put('/tenant/tremendous-credentials', requireAuth, async (req, res, next) => {
  ...
  const { api_key, funding_source_id } = parsed.data;
    ...
    const result = await withUserTransaction(req.userId, async (client) => {
      ...
      const { rows: [row] } = await client.query(
        `update tenants
         ...
         where id = $4
         returning tremendous_funding_source_id, tremendous_connected_at`,
        [api_key, key, funding_source_id, ctx.tenant_id]
      );
      return row;
    });

    res.json({ configured: true, funding_source_id: result.tremendous_funding_source_id, connected_at: result.tremendous_connected_at });
  } catch (err) {
    next(err);
  }
});
