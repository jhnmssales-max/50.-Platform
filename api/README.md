...
**Not yet verified against Stripe's real API** — `api.stripe.com` is unreachable from this sandbox (confirmed directly). Every scenario above, including webhook signature verification, is verified against this project's own local stand-ins: a Stripe API stub for `checkout.session.create`/`payment_intents.retrieve`, and this project's own implementation of Stripe's publicly documented HMAC signing scheme (not a signature Stripe itself produced). First real verification happens post-deploy, against a real Stripe test-mode account.
...
**Stripe billing** (see `supabase/README.md`'s billing-schema migration entry) is Stages 1-3 of a four-stage build: the one-time activation checkout, the full webhook handling (activation — both immediate and delayed-ACH-clearing — plus failed-payment and dispute suspension), and close/reopen all exist now. Only Stage 4 remains: the reward-issuance worker itself — selecting eligible referrals, charging the $X-per-referral fee off-session, and *then* calling the gift-card provider, never before the charge clears. One gap not deferred to a later stage but genuinely open now: `tenants_select_own`'s RLS policy grants a tenant's *entire* row — including the billing/pricing columns (`platform_fee_cents`, `monthly_spend_cap_cents`, etc.) — to any staff member, dealer or admin alike, since Postgres RLS can't restrict individual columns within one policy. Mitigated at the API layer for now (every route builds its response from named fields, never `select *` or a spread of a raw `tenants` row or `getCallerContext`'s own ctx object — see its own comment in `src/db.js`), with `stripe_customer_id`/`stripe_payment_method_id` specifically never appearing in any response body; a security-barrier view or column-level grants would close this at the database level too, deliberately deferred as not worth the complexity yet.

**No part of Stripe billing has been verified against Stripe's real API** — `api.stripe.com` is unreachable from this sandbox, confirmed directly. Every stage so far is verified against local stand-ins only: a stub server for the two Stripe endpoints this project calls, and this project's own implementation of Stripe's publicly documented webhook-signing algorithm (never a signature Stripe itself produced). First real verification happens post-deploy, against a real Stripe test-mode account.
