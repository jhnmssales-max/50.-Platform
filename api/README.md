...
`409` if the tenant has already completed activation (`activation_paid_at` is already set) — this doesn't create a second charge. Requires `FRONTEND_BASE_URL` to be set (used to build the Checkout redirect URLs); there's no fallback the way the invite email has one, since Stripe Checkout can't function without a real redirect URL the way an email can still show a non-clickable link as plain text.

```json
{ "url": "https://checkout.stripe.com/c/pay/cs_...", "session_id": "cs_..." }
```

Note: this stage doesn't include the `/billing/activation-success.html` / `/billing/activation-cancelled.html` frontend pages the redirect URLs point at — no frontend page was requested for this stage, so those URLs are only as real as `FRONTEND_BASE_URL` + a page that doesn't exist yet. Worth building before this goes live for a real dealer.
...
Handles `checkout.session.completed` today: on `payment_status: 'paid'`, stores `stripe_customer_id` and `stripe_payment_method_id` (fetched via a follow-up `GET /v1/payment_intents/:id`, since the webhook payload only gives that id as a string) on the tenant named by the session's `client_reference_id`, and sets `billing_status = 'active'` and `activation_paid_at = now()`. Idempotent on `activation_paid_at IS NULL` — Stripe redelivers events, and a redelivery of an already-processed one is a silent no-op, not a second write or an error.

**Known gap, not silently papered over:** a `checkout.session.completed` event can fire with `payment_status: 'unpaid'` for a delayed-notification payment method — `us_bank_account` (ACH) specifically, which is exactly the payment method the $500+ per-referral tier requires. This handler correctly refuses to activate on anything but `payment_status: 'paid'`, but doesn't yet listen for `checkout.session.async_payment_succeeded` / `async_payment_failed`, so a bank-account tenant whose Checkout completes before their debit clears currently has no terminal outcome recorded until a future stage adds those two event types to this same handler.

Every other event type is acknowledged (`200`) without action — Stripe retries a non-2xx with backoff, which only makes sense for an event this handler actually intends to act on. Stage 3 adds `payment_intent.payment_failed` and `charge.dispute.created` as more `case`s in this same `switch`, not a new route.
...
**Stripe billing** (see `supabase/README.md`'s billing-schema migration entry for Stage 1's schema) is Stage 2 of a four-stage build: the one-time activation checkout and its `checkout.session.completed` webhook handling exist now. Not yet built: Stage 3 (`payment_intent.payment_failed`/`charge.dispute.created` — same webhook, more `case`s) and Stage 4 (the reward-issuance worker itself — selecting eligible referrals, charging the $X-per-referral fee off-session, and *then* calling the gift-card provider, never before the charge clears). Two gaps specific to what's built so far, not deferred to a later stage but genuinely open now: a `us_bank_account` tenant whose Checkout session completes before their ACH debit clears has no terminal outcome recorded until `checkout.session.async_payment_succeeded`/`async_payment_failed` handling is added to the webhook; and `tenants_select_own`'s RLS policy grants a tenant's *entire* row — including the new billing/pricing columns (`platform_fee_cents`, `monthly_spend_cap_cents`, etc.) — to any staff member, dealer or admin alike, since Postgres RLS can't restrict individual columns within one policy. Mitigated at the API layer for now (every route builds its response from named fields, never `select *` or a spread of a raw `tenants` row or `getCallerContext`'s own ctx object — see its own comment in `src/db.js`), with `stripe_customer_id`/`stripe_payment_method_id` specifically never appearing in any response body; a security-barrier view or column-level grants would close this at the database level too, deliberately deferred as not worth the complexity yet.
